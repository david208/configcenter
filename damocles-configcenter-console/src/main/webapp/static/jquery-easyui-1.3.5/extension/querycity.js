(function($){
    $.querycity = function(input,options){
	    ...
	    popContainer.find('a').bind('click',function(){
		    input.val($(this).html());
		    onSelect($(this).html());
		    popMain.hide();
        ...
        suggestMain.find('.list_city_container a').bind('click',function(){
        ...
        suggestMain.find('.page_break a').bind('mouseover',function(){
            t_suggest_focus = true;        
        }).bind('mouseout',function(){
            t_suggest_focus = false;
        });
	    suggestMain.find('.page_break a').bind('click',function(event){
        ...
        function popInit(){
    		...
    		for(var itemLabel in options.tabs){		
			    ...
			    for(var item in options.tabs[itemLabel]){
				    ...
				    var cityInfo = options.tabs[itemLabel][item];
				    /*if(typeof(cityInfo)==="undefined"){
					    break;
    			    }else{
    			    }*/				
				    if(cityInfo.name){
				    	$("#label_"+input.attr('id')+index+"_container").append("<li><a href='javascript:void(0)'>"+cityInfo.name+"</a></li>");
				    }
			    }
		    }			
        }
...
$.fn.bgIframe = $.fn.bgiframe = function(s) {
	if (  /6.0/.test(navigator.userAgent) ) {
		...
		iframe.setAttribute("style","display:block;position:absolute;z-index:-1;"+(s.opacity !== false?'filter:Alpha(Opacity=\'0\');':'')+
